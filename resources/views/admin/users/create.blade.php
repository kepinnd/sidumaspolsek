        ...
        <form method="POST" action="{{ route('admin.users.store') }}">
            ...
            <div class="mb-4">
                ...
                <select id="role" name="role" required
                    ...
                    <option value="kapolsek" {{ old('role') == 'kapolsek' ? 'selected' : '' }}>Kapolsek</option>
                    <option value="petugas_spkt" {{ old('role') == 'petugas_spkt' ? 'selected' : '' }}>Petugas SPKT</option>
            ...
            <div id="masyarakat_fields" style="display: none;"> {{-- Default ke none, JS akan handle --}}
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('no_telp') border-red-500 @enderror"
                           id="no_telp" type="text" name="no_telp" value="{{ old('no_telp') }}">
                    @error('no_telp')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Panggil saat load untuk menangani old('role')
